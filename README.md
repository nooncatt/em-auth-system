# Custom Authentication & Authorization System (Test Task)

Backend-приложение на **Django + DRF**, реализующее собственную систему **аутентификации и авторизации** без использования встроенного Django `auth`.  
Проект демонстрирует работу с JWT-токенами, обработку ролей, правил доступа (RBAC) и выдачу ресурсов на основе разрешений.

## Цели проекта

- Создать собственную модель пользователя.
- Реализовать регистрацию, логин, логаут, обновление профиля и мягкое удаление.
- После логина — определять пользователя на всех последующих запросах.
- Построить свою систему разграничения прав (RBAC): роли → бизнес-элементы → разрешения.
- Реализовать выдачу **401 Unauthorized** и **403 Forbidden** согласно правилам.
- Создать Mock-ресурсы и показать, как система авторизации ограничивает доступ.
- Сделать API, через которое **администратор** может управлять правилами доступа.

## Стек технологий

- Python 3  
- Django  
- Django REST Framework  
- bcrypt — хеширование паролей  
- PyJWT — токены авторизации  
- PostgreSQL

## Функционал системы

### 1. Пользователи
- Регистрация (ФИО, email, пароль, повтор пароля)  
- Вход по email + пароль  
- Выдача JWT-токена  
- Определение пользователя по `Authorization: Bearer <token>`  
- Обновление профиля  
- Мягкое удаление (`is_active = False`)  
- Logout (удаление токена на стороне клиента)

### 2. Система разграничения прав (RBAC)

Используются таблицы:

#### Role
Роли пользователей: `admin`, `manager`, `user`, `guest`, …

#### BusinessElement
Элементы приложения: `users`, `products`, `orders`, `access_rules`

#### AccessRule
Разрешения роли для элемента:

| Поле | Описание |
|------|----------|
| can_read | читать свои |
| can_read_all | читать все |
| can_create | создавать |
| can_update | обновлять свои |
| can_update_all | обновлять любые |
| can_delete | удалять свои |
| can_delete_all | удалять любые |

Поведение системы:

- Нет токена → **401 Unauthorized**  
- Есть токен, но нет разрешения → **403 Forbidden**  
- Есть права → **200 OK**

### 3. Mock-ресурсы

Минимальные фейковые объекты, демонстрирующие работу системы:

- `GET /api/products/`  
- `GET /api/orders/`

Они не используют БД — только проверку прав.


## Структура проекта
```markdown
em-auth-system/      
|
|-- config/          # Django-проект (global settings, URLs, middleware)
| |-- settings.py    # настройки: БД, приложения, DRF, middleware
| |-- urls.py        # глобальная маршрутизация всего API (подключает users и mockapp)
| -- middleware.py   # JWT middleware (разбор токена, request.user)
|
|-- users/           # приложение пользователей: модели, логин, регистрация, роли, права
| |-- models.py      # модели БД
| |-- serializers.py # DRF-сериализаторы: регистрация, логин, профиль
| |-- views.py       # API-views
| |-- urls.py        # маршрутизация приложения users
| |-- services.py    # логика JWT + bcrypt (хэширование пароля, генерация токена)
| -- permissions.py  # проверка прав доступа
|
|-- mockapp/         # mock-приложение для ресурсов
| |-- views.py       # simple views: /products и /orders
| -- urls.py         # маршруты ресурсов
|
|-- README.md        
|-- requirements.txt # cписок зависимостей
-- manage.py
```
## Схема БД
```
User
├─ id
├─ full_name
├─ email (unique)
├─ password_hash
├─ role_id → Role.id
├─ is_active (soft delete)
└─ timestamps

Role
├─ id
└─ name

BusinessElement
├─ id
├─ code
└─ name

AccessRule
├─ id
├─ role_id → Role
├─ element_id → BusinessElement
├─ can_read
├─ can_read_all
├─ can_create
├─ can_update
├─ can_update_all
├─ can_delete
└─ can_delete_all
```
## 🔑 JWT аутентификация

Токен создаётся после логина и передаётся:
Authorization: Bearer <token>

Кастомный middleware:
- извлекает токен  
- декодирует JWT  
- ищет пользователя  
- записывает в `request.user`

## 🛠 Запуск проекта

```bash
git clone <url>
cd em-auth-system

pip install -r requirements.txt
python manage.py migrate
python manage.py runserver
```
Примеры поведения
```bash

Без токена
GET /api/products/
→ 401 Unauthorized

С токеном без прав
→ 403 Forbidden

С токеном admin
→ 200 OK
[
  {"id": 1, "name": "Product 1"},
  {"id": 2, "name": "Product 2"}
]

🛡 Admin API

Только роль admin может управлять:
- ролями
- бизнес-элементами
- правилами доступа
```